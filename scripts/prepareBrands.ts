...
const brands: CraftBrand[] = []

// Note: brands -> companies are not translated

const duplicateBrands: Record<number, number> = {}
function combineBrands(localeBrand: CraftBrand) {
...
(enCraftBrands as unknown as CraftBrand[]).forEach(combineBrands)
// Include unique brands that are only on Korean site
